...
const register = async (req, res) => {
    try {
        ...
        const randomInt = await (0, help_1.dbRandomInt)();
...
const sendResetUrl = async (req, res) => {
    try {
        const { email } = req.body;
        const user = await prisma.user.findUnique({
            where: { email },
        ...
        const getRandomStr = (0, help_1.generateRandomStr)(128);
        await prisma.user.update({
            where: { email },
            data: {
...
const resetPwd = async (req, res) => {
    try {
        const { pwd, email, reset_token } = req.body;
        const hashedPwd = await bcrypt.hash(pwd, 10);
        const user = await prisma.user.findUnique({
            where: { email },
        });
...
const refreshToken = async (req, res) => {
    try {
        ...
        jwt.verify(refreshToken, REF_TOKEN_SECRET, (err, decoded) => {
            if (err)
